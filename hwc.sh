...
# Version: 6.5.7 (Docker Inspect Template Syntax Fix)

# --- 第1節:全域設定與定義 ---
set -eo pipefail

# 顏色定義,用於日誌輸出
FontColor_Red="\033[31m"; FontColor_Green="\03_3[32m"; FontColor_Yellow="\033[33m"
...
# ... (从 self_install 到 generate_caddy_config 的所有函数保持不变) ...
# 为了简洁，这里省略了未改动的函数，替换时请使用您已有的版本

self_install() {
    local args_string; printf -v args_string '%q ' "$@"; local running_script_path
    ...
    if ! command -v curl &>/dev/null; then
        log WARN "'curl' 未安裝,正在嘗試安裝..."; if command -v apt-get &>/dev/null; then apt-get update && apt-get install -y --no-install-recommends curl; fi
        if command -v yum &>/dev/null || command -v dnf &>/dev/null; then command -v yum &>/dev/null && yum install -y curl; command -v dnf &>/dev/null && dnf install -y curl; fi
    fi
    if curl -sSL "${SCRIPT_URL}" -o "${SHORTCUT_PATH}"; then chmod +x "${SHORTCUT_PATH}"; log INFO "快捷命令 'hwc' 安裝成功。正在從新位置重新啟動..."; exec "${SHORTCUT_PATH}" $args_string
    else log ERROR "無法安裝 'hwc' 快捷命令至 ${SHORTCUT_PATH}。"; log WARN "本次將臨時運行腳本,請檢查權限後重試。"; sleep 3; fi
}
...
detect_cert_path() {
    local domain="$1"; local base_path="/data/caddy/certificates"; if container_exists "$CADDY_CONTAINER_NAME"; then
        for ca_dir in "acme-v02.api.letsencrypt.org-directory" "acme.zerossl.com-v2-DV90"; do
            local cert_check; cert_check=$(docker exec "$CADDY_CONTAINER_NAME" sh -c "[ -f $base_path/$ca_dir/$domain/$domain.crt ] && echo 'exists'" 2>/dev/null)
            if [ "$cert_check" = "exists" ]; then echo "$base_path/$ca_dir/$domain/$domain.crt|$base_path/$ca_dir/$domain/$domain.key"; return 0; fi
        done
    fi; echo "$base_path/acme-v02.api.letsencrypt.org-directory/$domain/$domain.crt|$base_path/acme-v02.api.letsencrypt.org-directory/$domain/$domain.key"; return 1
}
...
generate_caddy_config() {
    local primary_domain="$1" email="$2" log_mode="$3" proxy_domain="$4" backend_service="$5"; mkdir -p "${CADDY_CONFIG_DIR}"; local global_log_block=""
    if [[ ! "$log_mode" =~ ^[yY]$ ]]; then global_log_block=$(cat <<-'GLOBALLOG'
...
(security_headers) { header -Via; header -Server; header Server "nginx"; }
(proxy_to_backend) { reverse_proxy ${backend_service} { header_up Host {args.0}; header_up X-Real-IP {remote}; header_up X-Forwarded-For {remote}; header_up X-Forwarded-Proto {scheme}; } }
EOF
    if [ -n "$primary_domain" ]; then cat >> "${CADDY_CONFIG_FILE}" <<EOF
${primary_domain} { import security_headers; import proxy_to_backend {host}; }
EOF
; fi; local proxy_target="${primary_domain:-${backend_service}}"; cat >> "${CADDY_CONFIG_FILE}" <<EOF
${proxy_domain} { import security_headers; import proxy_to_backend ${proxy_target}; }
EOF
...
generate_singbox_config() {
    ...
    if container_exists "$ADGUARD_CONTAINER_NAME" && [ "$(docker inspect -f '{{.State.Running}}' "$ADGUARD_CONTAINER_NAME" 2>/dev/null)" = "true" ]; then
        local AG_IP
        # [FIXED] 使用 index 函数处理带连字符的网络名
        AG_IP=$(docker inspect -f "{{ index .NetworkSettings.Networks \"${SHARED_NETWORK_NAME}\" \"IPAddress\" }}" "$ADGUARD_CONTAINER_NAME" 2>/dev/null)
    ...
    cat > "${SINGBOX_CONFIG_FILE}" <<EOF
{
  ...
  "endpoints": [ { "type": "wireguard", "tag": "warp-out", "system": false, "mtu": 1280, "address": [ "${ipv4_address}/32", "${ipv6_address}/128" ], "private_key": "${private_key}", "peers": [ { "address": "engage.cloudflareclient.com", "port": 2408, "public_key": "${public_key}", "allowed_ips": [ "0.0.0.0/0", "::/0" ], "persistent_keepalive_interval": 25 } ] } ],
  ...
  "outbounds": [ { "type": "direct", "tag": "direct", "tcp_fast_open": true, "tcp_multi_path": false, "udp_fragment": false, "connect_timeout": "5s" } ],
...
# ... (从 manage_caddy 到 uninstall_all_services 的所有函数保持不变) ...
# 为了简洁，这里省略了未改动的函数，替换时请使用您已有的版本
...
cleanup_and_recreate_network() {
    ...
    for container in "${containers_to_process[@]}"; do
        if container_exists "$container"; then
            ...
            docker stop "$container" &>/dev/null || log WARN "無法停止 $container"
    ...
    if [[ " ${found_containers[*]} " =~ " ${SINGBOX_CONTAINER_NAME} " ]]; then
        ...
        if container_exists "$ADGUARD_CONTAINER_NAME" && [ "$(docker inspect -f '{{.State.Running}}' "$ADGUARD_CONTAINER_NAME")" = "true" ]; then
            local NEW_AG_IP
            # [FIXED] 使用 index 函数处理带连字符的网络名
            NEW_AG_IP=$(docker inspect -f "{{ index .NetworkSettings.Networks \"${SHARED_NETWORK_NAME}\" \"IPAddress\" }}" "$ADGUARD_CONTAINER_NAME" 2>/dev/null)
...
# ... (check_all_status 和 start_menu 函数保持不变) ...
