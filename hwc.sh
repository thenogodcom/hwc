#!/usr/bin/env bash
#
# Description: Ultimate All-in-One Manager for Caddy & Sing-box (AdGuard Removed)
# Author: Refactored from v6.7.0
# Version: 6.7.1 (Based on v6.7.0, Removed AdGuard Home)
...
create_shared_network() {
    if ! docker network inspect "${SHARED_NETWORK_NAME}" &>/dev/null; then
        ...
        if ! docker network create --subnet="${SHARED_NETWORK_SUBNET}" "${SHARED_NETWORK_NAME}"; then
            log ERROR "創建共享網絡 ${SHARED_NETWORK_NAME} 失敗！"
            return 1
        fi
        log INFO "共享網絡 ${SHARED_NETWORK_NAME} (子網: ${SHARED_NETWORK_SUBNET}) 創建成功。"
    fi
    return 0
}
...
EOF
    log INFO "已為域名 ${proxy_domain}$([ -n "$primary_domain" ] && echo " 和 ${primary_domain}") 建立 Caddyfile，所有 TCP 流量將代理到 app:80。"
}
...
generate_singbox_config() {
    ...
    # [修正] 由於移除了 AdGuard，直接使用 Cloudflare DNS 作為預設
...
restart_all_services() {
    log INFO "正在按依賴順序重啟容器: Caddy (SSL/Proxy) -> Sing-box (Core)..."
    local restart_order=("$CADDY_CONTAINER_NAME:Caddy" "$SINGBOX_CONTAINER_NAME:Sing-box")
    ...
    for item in "${restart_order[@]}"; do
        ...
        if container_exists "$container" && [ "$(docker inspect -f '{{.State.Status}}' "$container" 2>/dev/null)" = "running" ]; then
            log INFO "--------------------------------------------------"
            log INFO "正在重啟 ${service_name}..."
            if docker restart "$container" &>/dev/null; then
                restarted_count=$((restarted_count + 1))
                wait_for_container_ready "$container" "$service_name" 20
                if [ "$container" = "$CADDY_CONTAINER_NAME" ]; then
...
uninstall_all_services() {
    log WARN "此操作將刪除 Caddy 和 Sing-box 的所有相關數據！";
    read -p "您確定要徹底清理所有服務嗎? (y/N): " choice < /dev/tty;
    if [[ ! "$choice" =~ ^[yY]$ ]]; then log INFO "操作已取消。"; return; fi;
    log INFO "正在停止並刪除所有服務容器...";
    local containers_to_remove=("$CADDY_CONTAINER_NAME" "$SINGBOX_CONTAINER_NAME");
    for name in "${containers_to_remove[@]}"; do
        if container_exists "$name"; then docker stop "$name" &>/dev/null; docker rm "$name" &>/dev/null; fi
    done
    log INFO "正在刪除本地設定檔和數據...";
    rm -rf "${APP_BASE_DIR}";
    docker volume rm "${CADDY_DATA_VOLUME}" &>/dev/null || true;
    docker network rm "${SHARED_NETWORK_NAME}" &>/dev/null || true;
    log INFO "所有服務已徹底清理完畢。";
}

check_all_status() { local containers=("$CADDY_CONTAINER_NAME" "$SINGBOX_CONTAINER_NAME"); for container in "${containers[@]}"; do if ! container_exists "$container"; then CONTAINER_STATUSES["$container"]="${FontColor_Red}未安裝${FontColor_Suffix}"; else local status; status=$(docker inspect --format '{{.State.Status}}' "$container" 2>/dev/null); if [ "$status" = "running" ]; then CONTAINER_STATUSES["$container"]="${FontColor_Green}運行中${FontColor_Suffix}"; else CONTAINER_STATUSES["$container"]="${FontColor_Red}異常 (${status})${FontColor_Suffix}"; fi; fi; done; }

start_menu() { while true; do check_all_status; clear; echo -e "\n${FontColor_Purple}Caddy + Sing-box 終極管理腳本${FontColor_Suffix} (v${SCRIPT_VERSION:-6.7.1})"; echo -e "  快捷命令: ${FontColor_Yellow}hwc${FontColor_Suffix}  |  設定目錄: ${FontColor_Yellow}${APP_BASE_DIR}${FontColor_Suffix}"; echo -e " --------------------------------------------------"; echo -e "  Caddy 服務        : ${CONTAINER_STATUSES[$CADDY_CONTAINER_NAME]}"; echo -e "  Sing-box 服務     : ${CONTAINER_STATUSES[$SINGBOX_CONTAINER_NAME]}"; echo -e " --------------------------------------------------\n"; echo -e " ${FontColor_Green}1.${FontColor_Suffix} 管理 Caddy..."; echo -e " ${FontColor_Green}2.${FontColor_Suffix} 管理 Sing-box (整合核心服務)...\n"; echo -e " ${FontColor_Yellow}3.${FontColor_Suffix} 清理日誌並重啟所有服務"; echo -e " ${FontColor_Red}4.${FontColor_Suffix} 徹底清理所有服務\n"; echo -e " ${FontColor_Yellow}0.${FontColor_Suffix} 退出腳本\n"; read -p " 請輸入選項 [0-4]: " num < /dev/tty; case "$num" in 1) manage_caddy;; 2) manage_singbox;; 3) clear_logs_and_restart_all; press_any_key;; 4) uninstall_all_services; press_any_key;; 0) exit 0;; *) log ERROR "無效輸入!"; sleep 2;; esac; done; }
...
EOM
echo -e "${FontColor_Purple}Caddy + Sing-box 終極一鍵管理腳本${FontColor_Suffix} (v${SCRIPT_VERSION})"
echo "----------------------------------------------------------------"
